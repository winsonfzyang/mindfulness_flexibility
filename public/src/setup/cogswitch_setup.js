...
function finish_cogswitch_save() {
    $.ajax({
        type: "POST",
        url: "/exp1-data",
        data: JSON.stringify(jsPsych.data.get().values()),
        ...
        .fail(function() {
            ...
            var filename = jsPsych.data.get().values()[0].ID_DATE + "_cogswitch_" + jsPsych.data.get().values()[0].daynumber + ".csv";
...
function start_cogswitch() {
    ...
    jsPsych.init({
        ...
        on_close: function() {
            close_cogswitch_save()
        },
        on_finish: function() {
            finish_cogswitch_save()
            // jsPsych.data.displayData();
        }
    });
}
