...
function finish_affswitch_save() {
    $.ajax({
        type: "POST",
        url: "/exp1-data",
        data: JSON.stringify(jsPsych.data.get().values()),
        ...
        .fail(function() {
            ...
            var filename = jsPsych.data.get().values()[0].ID_DATE + "_affswitch_" + jsPsych.data.get().values()[0].daynumber + ".csv";
...
function start_affswitch() {
    ...
    jsPsych.init({
        ...
        on_close: function() {
            close_affswitch_save()
        },
        on_finish: function() {
            finish_affswitch_save()
            // jsPsych.data.displayData();
        }
    });
}
